...
var _configRule = require("./config-rule");

var _configRule2 = _interopRequireDefault(_configRule);
...
var debug = require("debug")("eslint:autoconfig");
...
Registry.prototype = {
    ...
    populateFromCoreRules: function populateFromCoreRules() {
        var rulesConfig = _configRule2.default.createCoreRuleConfigs();
    ...
    buildRuleSets: function buildRuleSets() {
        ...
        var addRuleToRuleSet = function (rule) {
            ...
            if (this.rules[rule][idx] && (hasFewCombos || this.rules[rule][idx].specificity <= 2)) {
                 ...
                 */
                if (!hasFewCombos && _typeof(this.rules[rule][idx].config[1]) === "object") {
                    return;
                }
     ...
     */
    filterBySpecificity: function filterBySpecificity(specificity) {
        var ruleIds = Object.keys(this.rules),
            newRegistry = new Registry();
     ...
     */
    lintSourceCode: function lintSourceCode(sourceCodes, config, cb) {
        var ruleSetIdx = void 0,
            lintedRegistry = void 0;

        lintedRegistry = new Registry();
        lintedRegistry.rules = Object.assign({}, this.rules);

        var ruleSets = lintedRegistry.buildRuleSets();

        lintedRegistry = lintedRegistry.stripExtraConfigs();

        debug("Linting with all possible rule combinations");

        var filenames = Object.keys(sourceCodes);
        var totalFilesLinting = filenames.length * ruleSets.length;
        ...
        filenames.forEach(function (filename) {
            ...
            ruleSets.forEach(function (ruleSet) {
                var lintConfig = Object.assign({}, config, { rules: ruleSet });
                var lintResults = _eslint2.default.verify(sourceCodes[filename], lintConfig);
