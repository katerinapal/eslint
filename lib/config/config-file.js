...
var _configValidator = require("./config-validator");
...
var _pathUtil = require("../util/path-util");

var _pathUtil2 = _interopRequireDefault(_pathUtil);

var _moduleResolver = require("../util/module-resolver");
...
var debug = require("debug")("eslint:config-file");
...
var CONFIG_FILES = [".eslintrc.js", ".eslintrc.yaml", ".eslintrc.yml", ".eslintrc.json", ".eslintrc", "package.json"];

var resolver = new _moduleResolver2.default();
...
function loadConfigFile(file) {
    ...
    return _configOps2.default.merge(_configOps2.default.createEmptyConfig(), config);
}
...
function applyExtends(config, filePath, relativeTo) {
    ...
    config = configExtends.reduceRight(function (previousValue, parentPath) {
        ...
        if (parentPath === "eslint:recommended") {
             ...
             */
            parentPath = !path.isAbsolute(parentPath) ? path.join(relativeTo || path.dirname(filePath), parentPath) : parentPath;
        }

        try {
            debug("Loading " + parentPath);
            return _configOps2.default.merge(load(parentPath, false, relativeTo), previousValue);
        } catch (e) {
...
function normalizePackageName(name, prefix) {
     ...
     */
    if (name.indexOf("\\") > -1) {
        name = _pathUtil2.default.convertPathToPosix(name);
    }
...
function resolve(filePath, relativeTo) {
    if (isFilePath(filePath)) {
        ...
        var normalizedPackageName = void 0;
...
function load(filePath, applyEnvironments, relativeTo) {
    ...
    var config = loadConfigFile(resolvedPath);
    ...
    if (config) {
        ...
        _configValidator2.default.validate(config, filePath);
...
exports.default = {
    ...
    getFilenameForDirectory: function getFilenameForDirectory(directory) {
        for (var i = 0, len = CONFIG_FILES.length; i < len; i++) {
            var filename = path.join(directory, CONFIG_FILES[i]);
