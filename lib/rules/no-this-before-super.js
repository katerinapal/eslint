    ...
    create: function create(context) {
        ...
        return {
            ...
            onCodePathStart: function onCodePathStart(codePath, node) {
                if (isConstructorFunction(node)) {
                    ...
                    funcInfo = {
                        upper: funcInfo,
                        isConstructor: true,
                        hasExtends: Boolean(classNode.superClass && !_astUtils2.default.isNullOrUndefined(classNode.superClass)),
                        codePath: codePath
                    };
                } else {
                    funcInfo = {
            ...
            Super: function Super(node) {
                if (!_astUtils2.default.isCallee(node) && isBeforeCallOfSuper()) {
                    setInvalid(node);
                }
            },
