...
function isTargetMethod(node) {
    return node.type === "MemberExpression" && TARGET_METHODS.test(_astUtils2.default.getStaticPropertyName(node) || "");
}
    ...
    while (node) {
        ...
        switch (parent.type) {
            ...
            case "ReturnStatement":
                {
                    var func = _astUtils2.default.getUpperFunction(parent);

                    if (func === null || !_astUtils2.default.isCallee(func)) {
                        return false;
                    }
                    node = func.parent;
                    break;
            ...
            //   list.every(function() {});
            case "CallExpression":
                if (_astUtils2.default.isArrayFromMethod(parent.callee)) {
        ...
        function checkLastSegment(node) {
            if (funcInfo.shouldCheck && funcInfo.codePath.currentSegments.some(isReachable)) {
                context.report({
                    ...
                    message: funcInfo.hasReturn ? "Expected to return a value at the end of this function." : "Expected to return a value in this function."
                });
            }
        }
