    ...
    create: function create(context) {
        ...
        return {
            "Program:exit": function ProgramExit() {
                ...
                // Check `parseInt()`
                variable = _astUtils2.default.getVariableByName(scope, "parseInt");
                ...
                variable = _astUtils2.default.getVariableByName(scope, "Number");
                if (!isShadowed(variable)) {
                    variable.references.forEach(function (reference) {
                        var node = reference.identifier.parent;

                        if (isParseIntMethod(node) && _astUtils2.default.isCallee(node)) {
                            checkArguments(node.parent);
                        }
                    });
                }
