...
var KEYS = _keywords2.default.concat(["as", "async", "await", "from", "get", "let", "of", "set", "yield"]);
...
exports.default = {
    ...
    create: function create(context) {
        ...
        function expectSpaceBefore(token, pattern) {
            ...
            if (prevToken && (CHECK_TYPE.test(prevToken.type) || pattern.test(prevToken.value)) && !isOpenParenOfTemplate(prevToken) && _astUtils2.default.isTokenOnSameLine(prevToken, token) && !sourceCode.isSpaceBetweenTokens(prevToken, token)) {
                context.report({
                    loc: token.loc.start,
        ...
        function expectSpaceAfter(token, pattern) {
            ...
            if (nextToken && (CHECK_TYPE.test(nextToken.type) || pattern.test(nextToken.value)) && !isCloseParenOfTemplate(nextToken) && _astUtils2.default.isTokenOnSameLine(token, nextToken) && !sourceCode.isSpaceBetweenTokens(token, nextToken)) {
                context.report({
                    loc: token.loc.start,
