...
exports.default = {
    ...
    create: function create(context) {
        ...
        var isParenthesised = _astUtils2.default.isParenthesised.bind(_astUtils2.default, sourceCode);
        var precedence = _astUtils2.default.getPrecedence;
        ...
        function isParenthesisedTwice(node) {
            ...
            return isParenthesised(node) && previousToken && nextToken && previousToken.value === "(" && previousToken.range[1] <= node.range[0] && nextToken.value === ")" && nextToken.range[0] >= node.range[1];
        }
        ...
        function requiresLeadingSpace(node) {
            ...
            var precededByKeyword = tokenBeforeLeftParen.type === "Keyword";
            ...
            return precededByKeyword && startsWithIdentifierPart || precededByUnaryPlus && startsWithUnaryPlus || precededByUnaryMinus && startsWithUnaryMinus;
        }
        ...
        function dryCallNew(node) {
            if (hasExcessParens(node.callee) && precedence(node.callee) >= precedence(node) && !(node.type === "CallExpression" && node.callee.type === "FunctionExpression" &&
        ...
        return {
            ...
            ExpressionStatement: function ExpressionStatement(node) {
                if (hasExcessParens(node.expression)) {
                    var firstTokens = sourceCode.getFirstTokens(node.expression, 2);
                    var firstToken = firstTokens[0];
                    var secondToken = firstTokens[1];

                    if (!firstToken || firstToken.value !== "{" && firstToken.value !== "function" && firstToken.value !== "class" && (firstToken.value !== "let" || !secondToken || secondToken.value !== "[")) {
            ...
            MemberExpression: function MemberExpression(node) {
                if (hasExcessParens(node.object) && precedence(node.object) >= precedence(node) && (node.computed || !(node.object.type === "Literal" && typeof node.object.value === "number" && _astUtils2.default.isDecimalInteger(node.object) ||

                // RegExp literal is allowed to have parens (#1589)
                node.object.type === "Literal" && node.object.regex)) && !((node.object.type === "FunctionExpression" || node.object.type === "ClassExpression") && isHeadOfExpressionStatement(node) && !hasDoubleExcessParens(node.object))) {
                    report(node.object);
                }
