        ...
        */
            ...
            if (node.right.type === "Identifier") {
                ...
                if (node.left.type === "ObjectPattern") {
                    node.left.properties.forEach(function (property) {
                        checkPropertyAccess(node.left, objectName, _astUtils2.default.getStaticPropertyName(property));
                    });
                }
            }
        }
        ...
        return {
            MemberExpression: function MemberExpression(node) {
                checkPropertyAccess(node, node.object && node.object.name, _astUtils2.default.getStaticPropertyName(node));
            },
                ...
                if (node.init && node.init.type === "Identifier") {
                    ...
                    if (node.id.type === "ObjectPattern") {
                        node.id.properties.forEach(function (property) {
                            checkPropertyAccess(node.id, objectName, _astUtils2.default.getStaticPropertyName(property));
                        });
                    }
                }
            },
