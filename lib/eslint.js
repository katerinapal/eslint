...
var _codePathAnalyzer = require("./code-path-analysis/code-path-analyzer");

var _codePathAnalyzer2 = _interopRequireDefault(_codePathAnalyzer);
...
var _commentEventGenerator = require("./util/comment-event-generator");

var _commentEventGenerator2 = _interopRequireDefault(_commentEventGenerator);

var _nodeEventGenerator = require("./util/node-event-generator");

var _nodeEventGenerator2 = _interopRequireDefault(_nodeEventGenerator);

var _sourceCode = require("./util/source-code");

var _sourceCode2 = _interopRequireDefault(_sourceCode);

var _traverser = require("./util/traverser");
...
function parseJsonConfig(string, location, messages) {
    ...
    try {
        ...
        if (_configOps2.default.isEverySeverityValid(items)) {
            return items;
        }
    } catch (ex) {}
...
function addDeclaredGlobals(program, globalScope, config) {
    var declaredGlobals = {},
        exportedGlobals = {},
        explicitGlobals = {},
        builtin = _environments2.default.get("builtin");

    Object.assign(declaredGlobals, builtin);

    Object.keys(config.env).forEach(function (name) {
        if (config.env[name]) {
            var env = _environments2.default.get(name),
...
function modifyConfigsFromComments(filename, ast, config, reportingConfig, messages) {
                    ...
                    case "eslint":
                        {
                            ...
                            Object.keys(items).forEach(function (name) {
                                ...
                                _configValidator2.default.validateRuleOptions(name, ruleValue, filename + " line " + comment.loc.start.line);
    ...
    Object.keys(commentConfig.env).forEach(function (name) {
        ...
        if (env) {
            commentConfig = _configOps2.default.merge(commentConfig, env);
        }
    });
    Object.assign(commentConfig.rules, commentRules);

    return _configOps2.default.merge(config, commentConfig);
}
...
function prepareConfig(config) {
    ...
    if (_typeof(config.env) === "object") {
        Object.keys(config.env).forEach(function (envName) {
            ...
            if (config.env[envName] && env && env.parserOptions) {
                parserOptions = _configOps2.default.merge(parserOptions, env.parserOptions);
            }
        });
    }
    ...
    var preparedConfig = {
        ...
        globals: _configOps2.default.merge({}, config.globals),
        env: _configOps2.default.merge({}, config.env || {}),
        settings: _configOps2.default.merge({}, config.settings || {}),
        parserOptions: _configOps2.default.merge(parserOptions, config.parserOptions || {})
    };
...
var exportedObject = function () {
    ...
    function parse(text, config, filePath) {
        ...
        try {
            ...
            var source = ex.lineNumber ? _sourceCode2.default.splitLines(text)[ex.lineNumber - 1] : null;
    ...
    api.verify = function (textOrSourceCode, config, filenameOrOptions, saveState) {
        ...
        if (text !== null) {
            ...
            if (text.trim().length === 0) {
                sourceCode = new _sourceCode2.default(text, blankScriptAST);
                return messages;
            }
            ...
            if (ast) {
                sourceCode = new _sourceCode2.default(text, ast);
            }
        } else {
        ...
        if (ast) {
            ...
            _configOps2.default.normalize(config);
            ...
            Object.keys(config.rules).filter(function (key) {
                ...
                ruleCreator = _rules2.default.get(key);
                ...
                if (!ruleCreator) {
                    ...
                    _rules2.default.define(key, ruleCreator);
                }
                ...
                try {
                    var ruleContext = new _ruleContext2.default(key, api, severity, options, config.settings, config.parserOptions, config.parser, ruleCreator.meta, parseResult && parseResult.services ? parseResult.services : {});
            ...
            traverser = new _traverser2.default();
            ...
            scopeManager = escope.analyze(ast, {
                ...
                fallback: _traverser2.default.getKeys
            });
            ...
            var eventGenerator = new _nodeEventGenerator2.default(api);

            eventGenerator = new _codePathAnalyzer2.default(eventGenerator);
            eventGenerator = new _commentEventGenerator2.default(eventGenerator, sourceCode);
    ...
    var defineRule = api.defineRule = function (ruleId, ruleModule) {
        _rules2.default.define(ruleId, ruleModule);
    };
